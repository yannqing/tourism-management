    ...
    stages {
        ...
        stage('构建镜像，创建并运行容器') {
            steps {
                ...
                sh "docker run -it --name ${APP_NAME} --restart=on-failure --network mynetwork -v /yannqing/${APP_NAME}/images:/yannqing/template/tourism-management/java/images -v /yannqing/${APP_NAME}:/yannqing/${APP_NAME} -p ${APP_PORT} -d ${APP_IMAGE}"
            }
        }
    }
}